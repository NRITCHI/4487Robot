...
const int diskIN1 = 13;                               // GPIO pin for rotating disk input 1
const int diskIN2 = 27;                               // GPIO pin for rotating disk input 2
const int ci_ServoPin1 = 4;                           // GPIO pin for servo 1
const int ci_Bucket = 12;                             // GPIO pin for servo 2 for bucket
const int ci_ServoChannel1 = 4;                       // PWM channel for servo motor
const int ci_BucketChannel = 5;                       // PWM channel for servo motor 2 for bucke
...
int toggle = 1;                                       // variable to track direction of dc motors
int curCheck;                                         // time of last check
int dumpDelay = millis();                             // base delay for dump
int dumpWait;                                         // time to check for dump delay
int position = 0;                                     // check what position sorting arm is in
int senseDelay;                                       // time delay for sensor
int pressed = 1;                                      // check if button is currently pressed
int dumping = 0;                                      // check if robot is currently dumping
...
void setup() {
  ...
  pinMode(diskIN1, OUTPUT);                           // configure pin mode for disk input 1
  pinMode(diskIN2, OUTPUT);                           // configure pin mode for disk input 2
  ledcWrite(ci_BucketChannel, degreesToDutyCycle(130)); // write bucket to correct position
...
void loop() {
  ...
  // colour detection and sort
   if (position == 0) {                                               // if in position 0 (at home position, looking for colour)
    if (r <= 13 && b <= 10 && g <= 10 && c <= 25) {                   // check if black (base colour)
      senseDelay = millis();                                          // start delay
    }else{
      if ((millis() - senseDelay) > 300){                             // if is longer than delay, check if is green
        if (r >= 12.52 && r <= 24.56 && b >= 6.95 && b <= 19.98 && g >= 11.10 && g <= 28.59 && c >= 31.44 && c <= 72.25) {
          curCheck = millis();                                        // update timer
          position = 1;                                               // change to position 1 (dump into robot)                                  
        }else {
          curCheck = millis();                                        // update timer
          position = 2;                                               // change to position 2 (dump out of robot)                                              
        }
      }
    }
  }
  ...
  if (position == 0) {
    ...
    ledcWrite(ci_ServoChannel1, degreesToDutyCycle(30));               // if position 1 change position to dump into robot
    position = 4;                                                      // change to position 4 (waiting position)
    curCheck = millis();                                               // reset timer
  } else if (position == 2) {
    ledcWrite(ci_ServoChannel1, degreesToDutyCycle(180));              // if position 2 change to send stone away from robot
    position = 4;                                                      // change to position 4 (waiting position)
    curCheck = millis();                                               // reset timer
  }

  if (position == 4) {                                                 // if position 4 (item discharged)
    if ((millis() - curCheck) > 130 && (millis() - curCheck) < 400) {  // wait in position for 130 millisecond      
      ledcWrite(ci_ServoChannel1, degreesToDutyCycle(95));             // go to home position
    } else if ((millis() - curCheck) >= 500) {                         // after 500 milliseconds
      position = 0;                                                    // return back to 0 (looking for colour)
      curCheck = millis();                                             // reset timer
    }
  }
  
  // code for bucket control
  ...
  if (dumping == 1 && (millis() - dumpDelay) > 1000){                  // if is starting, wait a second for anything to finish
    ledcWrite(ci_BucketChannel, degreesToDutyCycle(0));                // rotate bucket to dump
  ...
  if (curTime - lastTime > 10000) {                                    // wait ~10 ms
    ...
    if (toggle == 1) {                                                 // if toggle is at 1, drive sorting wheel and windmill forwards
      analogWrite(diskIN1, 78); 
      analogWrite(diskIN2, 0);                    
    } else if (toggle == -1) {                                         // if toggle is at -1, drive sorting wheel and windmill backwards
